...
# conclusion
##############
# Three observations about the titanic passengers that is revealed from 
# examining the chart above. 
# 1.More Female survivied in class 1 than male
# 2.More Female survivied in class 2 ,and the majority are between 20~40 years old
# 3.Majority of class 3 did not survived


# 2.

ggplot(data1,aes(age,fill=survived))+
  theme_bw()+
  scale_x_continuous(limits = c(0,12))+
  geom_histogram(bins = 30)+
  scale_color_manual(values = c('black','black'))+
  labs(title = 'Childreds (=< 12yrs) survival')+
  facet_grid(pclass~ gender)

##############
# conclusion
##############
# we can see that pclass 1 & 2 childreds survived with one exception
# pclass 3 childred are not so fortunate
# so, to some extend woman& childred first especially in pclass 1 & 2
